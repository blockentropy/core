...
func mlGenerate(model string, prompt string, negprompt string, seed string, signer sdk.AccAddress, blockHeight int64, machine string) {
	...
	for _, serverAddr := range serverList {
		...
		if err != nil {
			...
			message := fmt.Sprintf(`%s %s "%s" "%s" %s %s %s "%s"`, cmd, model, prompt, negprompt, seed, signer.String(), strconv.FormatInt(blockHeight, 10), machine)
...
func (k msgServer) Generate(goCtx context.Context, msg *types.MsgGenerate) (*types.MsgGenerateResponse, error) {
	...
	_ = ctx
	go mlGenerate(msg.Model, msg.Prompt, msg.Negprompt, msg.Seed, msg.GetSigners()[0], ctx.BlockHeight(), msg.Machine)
